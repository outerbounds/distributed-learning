...
import torchtune.training.checkpointing._utils as _torch_tune_utils

_torch_tune_utils.get_adapter_checkpoint_path = lambda *args, **kwargs: None
...
class AnamolyCollector:
    def __init__(self):
        self.data = []

    def add(self, condition: str, data_dict: Dict[str, Any]):
        self.data.append(
            {
                "condition": condition,
                "data": data_dict,
...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.checkpoint_epoch_frequency = cfg.get("checkpoint_epoch_frequency", 1)
        if self.checkpoint_epoch_frequency is not None:
            assert (
                self.checkpoint_epoch_frequency > 0
            ), "Checkpoint epoch frequency must be greater than 0"
    ...
    def train(self) -> None:
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            self.epochs_run += 1
            _intermediate_checkpoint = not (curr_epoch == self.total_epochs - 1)
            _current_epoch_valid = curr_epoch % self.checkpoint_epoch_frequency == 0
            if not _intermediate_checkpoint or _current_epoch_valid:
            ...
            # if self.checkpoint_epoch_frequency is not None and (not _intermediate_checkpoint or _current_epoch_valid):
